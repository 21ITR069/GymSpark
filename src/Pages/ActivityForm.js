...
const ActivityForm = () => {
  ...
  return (
    <>
      <div className="w-screen h-screen bg-gray-900 text-dark bg-light flex justify-center items-center">
        <form
          onSubmit={(e) => e.preventDefault()}
          className="form p-5 space-y-4 sm:w-1/2 w-4/12 bg-opacity-50 rounded-2xl border-red-800"
        >
          <h2 className="text-center text-3xl font-bold text-orange-500">Add Activity</h2>

          <input
            className="border-b-2 border-red-800 w-full bg-transparent text-dark h-10"
            value={sessionEmail}
            readOnly
          />
          <select
            className="border-b-2 border-red-800 w-full bg-transparent bg-dark text-dark h-10"
            name="activityType"
